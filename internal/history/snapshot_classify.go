// Truthful completeness and endpoint classification for Issue #33, per the
// Cache and snapshot invariant of Notes/PRD-sqloid.md: exclusive `complete`
// over the limited logical result, truthful `partial` and `truncated` that
// may coexist, count failure and short/empty observation endpoint rules, the
// unknown-remainder case, and no clamping of inconsistent count/cache
// evidence. Terminal outcomes stay orthogonal: classification consumes
// completeness facts only and never inspects the terminal outcome.
...
// when both logical endpoints are established, every row of the limited
// logical result is retained, no eviction occurred, and all count and page
// work finished; rows beyond the user's Limit are irrelevant because the
// known total already counts only the limited result.
func Classify(meta SnapshotMetadata, t TraversalFacts) Completeness {
	...
	// endpoint without eviction evidence) or count/page work did not finish,
	// or count/cache evidence is contradictory and cannot be trusted to be
	// complete.
	partial := !complete && (!highKnown || !workFinished || inconsistent ||
		(!meta.ReachedHigh && rowsBeyondRange))
