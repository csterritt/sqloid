...
- **TEXT** is the already-decoded string preserved verbatim through the quoting layer. Invalid UTF-8 was normalized once upstream by the shared `result.DecodeText` — exactly one U+FFFD per maximal invalid byte sequence per Unicode Table 3-7 (overlong `C0 80` as two one-byte subparts, surrogate `ED A0 80` as three, truncated E0–EF prefixes with valid second bytes as two-byte subparts, truncated F0–F4 prefixes with one/two/three valid continuation bytes as two/three-byte subparts, boundary second-byte constraints enforced for overlong/surrogate/above-U+10FFFF exclusion) — with `result.UTFWarning` metadata carried outside the payload; the CSV writer renormalizes nothing. A valid encoded U+FFFD (EF BF BD) is preserved unchanged and never sets the replacement signal (Issue #74).
...
- Issues #14 (typed literals, downstream of write/SQL paths), #22/#47 (shared typed result seam and output names), #23 (REAL/non-finite tokens), #33/#49 (snapshot metadata, immutable capture, warning exclusion), #50, #74 (corrected maximal-subpart decoding and valid U+FFFD preservation); JSON serialization remains with Issue #51.
