...
- `internal/ui/snapshot_finalize_test.go` — finalization cases over the model and metadata independence from later navigation.

Cross-references: Issues #24 (count), #30 (positional cache), #31 (byte cap), #33 (this work), #72 (settlement metadata retention); [concurrent-page-count.md](concurrent-page-count.md), [positional-result-cache.md](positional-result-cache.md), [byte-cap-oversized-values.md](byte-cap-oversized-values.md), [scoped-select-cancellation.md](scoped-select-cancellation.md), [serialized-vertical-paging.md](serialized-vertical-paging.md).
