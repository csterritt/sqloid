...
- **REAL**: finite values delegate to the single canonical `internal/result.RealToken` (Issue #85), so the shortest round-trip `strconv.FormatFloat(v, 'g', -1, 64)` token with `.0` appended iff it contains none of `.`, `e`, `E` — preserving REAL identity for integral values (`4` → `4.0`), negative zero (`-0.0`, sign-bit tested), subnormals (`5e-324`), and exponent output (`1e+20`) — is the one formatting-plus-suffix implementation shared by grid, CSV, JSON, and query/saved-SQL literals. Querybuilder keeps its typed non-finite rejection *before* the shared call: positive infinity, negative infinity, and NaN return a typed error instead of unsafe SQL, because `result.RealToken` intentionally returns display/export tokens (`Inf`, `-Inf`, `NaN`) for non-finite database values rather than rejecting them. The two policies stay distinct: query literals reject non-finite values; result formatting retains its established non-finite tokens.
...
### Issue #85 — centralized finite REAL token authority

Issue #85 removed the duplicate finite formatter (`realToken`) from `internal/querybuilder/value.go` and made `RenderSQLLiteral`'s finite REAL branch call `internal/result.RealToken`, so the formatting-plus-suffix algorithm has exactly one implementation in the repository (`internal/result/result.go`). The dependency direction is sound: `internal/querybuilder` imports `internal/result`, and `internal/result` imports neither `internal/querybuilder` nor any UI/driver package. Querybuilder's typed non-finite rejection stays *before* the shared call, preserving the distinct non-finite rules: query literals reject Inf/-Inf/NaN with a typed error and empty token, while result formatting (grid, CSV, JSON) retains its established `Inf`/`-Inf`/`NaN` tokens. Parsing, bound parameter types, text quoting, INTEGER/NULL/BLOB literals, result display, CSV/JSON serializers, and saved-SQL assembly are unchanged. Package-level evidence (cross-package behavioral parity tests, focused grep/static build check) is valid before Issue #57; shipped-TUI manual and end-to-end comparison must be rerun after Issue #57 lands.

## Testing

See the Issue #14 test catalog under [unit-tests.md](unit-tests.md); related contracts live in [schema-catalog.md](schema-catalog.md) (identifier provenance), [stale-schema-refresh.md](stale-schema-refresh.md) (fresh catalogs before quoting), and [early-integration-tracer.md](early-integration-tracer.md) (predecessor quoting rules now superseded by these shared atoms). Issue #85 added cross-package parity tests in `internal/querybuilder/sql_literal_test.go` (`TestRenderRealLiteralsMatchCanonicalRealToken` and `TestRenderRealLiteralsRejectNonFiniteWhileResultRetainsPolicy`) comparing finite REAL output from `RenderSQLLiteral` to `result.RealToken` for integral REAL `1.0`, negative zero, `1e+20`, the smallest subnormal, maximum finite float, and adjacent/precision-edge float64 values, with round-trip and locale-independence assertions and explicit non-finite rejection while result formatting retains its policy.

## References

- Issues #14 (universal parsing and SQL atoms), #40/#48 (destructive modal and saved-SQL consumers), #85 (centralized finite REAL token authority).
- `Notes/PRD-sqloid.md`: Numeric value parsing and rendering, SQL safety, Export formats and values.
- Related pages: [shared-typed-result-rendering.md](shared-typed-result-rendering.md) (the canonical `result.RealToken` site), [non-finite-real-grid.md](non-finite-real-grid.md) (non-finite display policy), [csv-export.md](csv-export.md), [json-export.md](json-export.md), [sql-save-targeting-serialization.md](sql-save-targeting-serialization.md).
