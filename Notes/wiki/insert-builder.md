...
`RunnableReport` (Issue #19, extended by Issue #67) evaluates INSERT in visual order: a zero-insertable-column table blocks at `RunFieldInsertColumns` with the exact reason `table has no insertable columns`; then every stored prompt is validated against the current `InsertableColumns` set before any completeness check — a prompt whose column is dropped, hidden, generated, or otherwise no longer insertable blocks at `RunFieldInsertColumns` with the exact reason `the insert column no longer exists` (Issue #67), regardless of its former choice or submitted value; then every prompted column in schema order must have exactly one choice (`complete the choice for column X`) and a submitted Value (`submit a value for column X`). All-omit is valid. A missing prompt state is an incomplete choice, so prompts must be begun for runnable data. The stale check iterates stored prompts in their seeded schema order and reports the first stale one, so multiple stale prompts are handled deterministically.

## Stale INSERT prompt rejection (Issue #67)

Issue #67 strengthens the authoritative INSERT runnable report so every stored prompt column must still exist in the current `InsertableColumns` set. A prompt made stale because its column was hidden, generated, dropped, or otherwise became non-insertable (while the table remains eligible) blocks execution with the specific stale-column reason `the insert column no longer exists` at `RunFieldInsertColumns`, before any per-column choice or submission completeness check on current prompts. The check builds the current insertable-column identity set from `InsertableColumns()` and validates every stored entry in `q.inserts` in seeded schema order; it reuses `InsertableColumns`, prompt identity, and report patterns already used for stale SET/WHERE state, and never infers insertability from declared types, defaults, names, or old prompt metadata.

`InsertSQL` and `InsertParams` (Issue #39 Task 4) were already gated on `RunnableReport().Runnable` before traversing stored prompts, so the strengthened report makes stale INSERT state emit no SQL and no parameters — no stale identifier is quoted and no former bound value escapes. Rejection is all-or-nothing through `RunnableReport`: the renderer never silently filters a stale prompt and renders the remainder, never quotes before validation, and never duplicates insertability checks. Accepted prompts preserve schema prompt order, Value/NULL/Default choices, parameter order, omission, and all-omit `DEFAULT VALUES` unchanged.
...
- `internal/querybuilder/insert_prompt_test.go` — prompt plan, closed choice set, hint scope, zero-column blocking, and Issue #67 stale-prompt blocking (dropped, hidden, generated columns) with current-prompt controls.
- `internal/querybuilder/insert_sql_test.go` — exact SQL/params: single Value, mixed Value/NULL/omit, empty TEXT, typed NULL, explicit NULL, DEFAULT VALUES, quoting, virtual tables, rejection of incomplete state, hint neutrality, and Issue #67 stale-prompt renderer refusal (empty SQL/nil params for dropped, hidden, generated, non-insertable, and multiple stale prompts) with current-state controls (ordered Value bindings, SQL NULL, Default/Omit exclusion, quoted identifiers, empty TEXT, typed NULL, all-omit DEFAULT VALUES).
- `internal/querybuilder/runnable_test.go` — Issue #67 `TestRunnableInsertGatesStalePrompts`: dropped, hidden, generated, and non-insertable stored prompts block at `RunFieldInsertColumns` with `the insert column no longer exists` before completeness checks, regardless of former choice (Value/NULL/Omit); multiple stale prompts report the first in schema order; stale blocks before incomplete current prompts; and current-prompt controls (same refresh, all-omit, surviving subset) remain runnable.
