...
### Count/cache contradiction at finalization (Issue #78)

Because the count and the cache are independent autocommit reads with no shared snapshot, drift is an accepted outcome. Issue #78 records one direction of that drift at finalization: when the count succeeds and its total falls **below** the retained cache end, the cache has retained rows the count says do not exist. `appendFinalizedResultEntry` derives `CountCacheInconsistent` from the same authoritative cache snapshot used for rows and range, before `SnapshotFacts`, and passes it through `Finalization.CountCacheInconsistent` into `TraversalFacts.CountCacheInconsistent`. Both facts — the successful known total and the retained range — are preserved **without clamping either**: the total is not raised to the retained end and the retained end is not lowered to the total. The corrected `history.Classify` from Issue #77 then rejects `complete` whenever the flag is set, reporting `partial` (plus `truncated` when independent eviction or known-overflow evidence coexists). Equality and lower retained ends do not set the flag — equality means the limited result is fully retained, and a lower retained end is the ordinary unknown remainder (count above the range). Pending, unavailable, failed, and cancelled counts never set the flag because no successful total exists to contradict the cache. Issue #80 extends the parity: the shared `deriveAuthoritativeFacts` helper in `internal/ui/snapshot_metadata.go` derives the contradiction identically for both `appendFinalizedResultEntry` and `activeExportFacts`, so the active export path now reports the contradiction's effect on completeness before finalization — identical active state produces equivalent endpoint/traversal facts and matching completeness labels on both paths. See [snapshot-metadata.md](snapshot-metadata.md) for the classification contract and [active-select-lifetime.md](active-select-lifetime.md) for the finalization seam.

### Limited-result classification boundary (Issue #79)

Issue #79 removes the dead `HasLimit`/`Limit` fields from `history.TraversalFacts`, `SnapshotFacts`, and `activeExportFacts`. The successful known total already counts the complete SELECT including the user's Limit (the count subquery wraps the entire `SelectSQL()` with the Limit inside it), so rows beyond that limited logical result are irrelevant and `history.Classify` consumes no raw builder Limit. The retained traversal inputs are exactly: `CountWorkFinished` (count request settled or never issued), `PageWorkFinished` (no page work outstanding or deferred), `ObservedShortFinalPage` (a final page actually observed to return fewer rows than requested, including an empty page — alone establishes the high endpoint when the count is unavailable or failed), and `CountCacheInconsistent` (contradictory count/cache evidence, preserved without clamping). Complete, partial, truncated, endpoint, and count/cache-inconsistency outcomes are unchanged; the classifier truth table now compares limited known-total cases against equivalent unbounded fact sets to prove identical labels. See [snapshot-metadata.md](snapshot-metadata.md) and [immutable-export-capture.md](immutable-export-capture.md).
...
See [unit-tests.md](unit-tests.md) and [source-code.md](source-code.md) for the complete catalogs. Later-page identities, viewport generations, and stale-response rejection are specified in [select-request-identities.md](select-request-identities.md) (Issue #26); cache behavior and paging navigation belong to later issues.
