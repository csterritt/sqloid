...
- **Classification preserved** — every result flows through Issue #6's `Settle` (late success discarded as cancelled; `context.Canceled` as cancelled; anything else failed) and Issue #7's post-error health verification; a lease-acquisition failure pre-settles through the Issue #60 classification ordering (cancellation → health → ordinary failure, documented in [prelease-cancellation.md](prelease-cancellation.md)) so callers always observe exactly one settlement.
