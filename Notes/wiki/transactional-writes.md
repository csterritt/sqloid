...
4. **Atomic pre-COMMIT cancellation check** — after statement success the write emits `WritePhaseCommitting`, and immediately before beginning COMMIT atomically inspects the request cancellation flag (`Request.CancelRequested`). **The cancellation flag wins even though the statement returned success**: a cancel requested at any cancellable point — before BEGIN, during execution, or after statement success — routes to rollback cleanup, never commit.
5. **Rollback cleanup or commit** — cancellation or any statement failure (including native constraint and trigger errors) enters `WritePhaseRollbackCleanup` and waits for the rollback; `RollbackConfirmed` is true exactly when the rollback succeeded. An uncancelled statement crosses exactly once to `tx.Commit()`. Rollback cleanup and commit are **noncancellable**: no interrupt is issued after the commit boundary, and a commit failure resolves (for #42) as a failed write with rollback attempted; persistence after a failed commit is unprovable and belongs to Issue #45's outcome-unknown workflow.
...
- Issues **#43** owns post-commit-boundary Ctrl+W/quit interaction and feedback rendering; **#45** owns unresolved rollback/commit (outcome-unknown) terminal handling and the non-persistence `RowsAffected` labeling.
