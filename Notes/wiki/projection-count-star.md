...
- `RemoveProjection(index)` — removes one committed entry by position; out-of-range indexes are ignored. Sentinels reappear through `ProjectionCandidates` as soon as removal empties the projection. Broad ordered-editing and duplicate-pair deduplication belong to Issue #16.
