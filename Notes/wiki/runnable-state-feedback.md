...
- **INSERT**: at least one insertable column (zero blocks with `table has no insertable columns`) → every stored prompt validated against the current `InsertableColumns` set (Issue #67: a prompt whose column is dropped, hidden, generated, or otherwise no longer insertable blocks with `the insert column no longer exists` at `RunFieldInsertColumns` before completeness checks, regardless of former choice) → every per-column prompt carrying exactly one {Value, NULL, Default/Omit} choice with submitted Value entries; **all-omit is valid**.
...
## SELECT renderer gating (Issue #66)

Issue #66 consolidates SELECT renderability onto this single runnable authority: every public SELECT-family SQL and parameter method — `SelectSQL`, `PageSQL`, `CountSQL`, `SelectParams`, `PageParams`, `CountParams` — requires `CommandSelect` **and** `RunnableReport().Runnable` before emitting anything. The shared `renderSelectCore` seam and `SelectParams` carry one `if q.command != CommandSelect || !q.RunnableReport().Runnable` gate; rejected states return only empty SQL and nil parameters, never partially rendered clauses or values, including cases whose component state is locally formattable. No second validator or renderer-specific validity interpretation exists. See [select-renderer-gating.md](select-renderer-gating.md) for the full rejected-output contract and unchanged valid SELECT/page/count semantics.

## Cross-references

Issues #15–#19, #65, #66, #67; forward to Issues #21 (pre-execution schema validation), #37/#39 (write flows), #38; the [where-guided-predicates.md](where-guided-predicates.md), [group-order-limit.md](group-order-limit.md), [projection-count-star.md](projection-count-star.md), [projection-ordered-editing.md](projection-ordered-editing.md), [select-renderer-gating.md](select-renderer-gating.md), [insert-builder.md](insert-builder.md), and [schema-validation-workflow.md](schema-validation-workflow.md) contracts; the Runnable-State Contract, Builder and Display Interaction, Global Key Precedence, QueryBuilder, UI, and Testing Decisions of `Notes/PRD-sqloid.md`.

## INSERT stale-prompt gating (Issue #67)

Issue #67 extends the INSERT branch of this single runnable authority: `reportInsert` now builds the current insertable-column identity set from `InsertableColumns()` and validates every stored entry in `q.inserts` in seeded schema order before any per-column choice or submission completeness check. A stored prompt whose column is dropped, hidden, generated, or otherwise no longer insertable (while the table remains eligible) returns `RunnableReport{Field: RunFieldInsertColumns, Reason: "the insert column no longer exists"}` regardless of its former choice or submitted value. Multiple stale prompts report the first in stored/schema order deterministically. The check reuses `InsertableColumns`, prompt identity, and report patterns already used for stale SET/WHERE state; it never infers insertability from declared types, defaults, names, or old prompt metadata. Because `InsertSQL` and `InsertParams` (Issue #39) were already gated on `RunnableReport().Runnable` before traversing stored prompts, stale INSERT state emits no SQL and no parameters — no stale identifier is quoted and no former bound value escapes. Rejection is all-or-nothing through `RunnableReport`; the renderer never silently filters a stale prompt, quotes before validation, or duplicates insertability checks. Accepted prompts preserve schema order, Value/NULL/Default choices, parameter order, omission, and all-omit `DEFAULT VALUES` unchanged. See [insert-builder.md](insert-builder.md) for the full INSERT contract.
