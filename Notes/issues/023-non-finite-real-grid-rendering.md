...
Define and implement grid rendering for non-finite REAL values (Inf, -Inf, NaN) that SQLite may return from pre-existing data. Issue 22 (merged with Issue 47's finite REAL token logic) covers finite REAL formatting only. The PRD states "Existing non-finite policy remains separate" and Issues 50/51 cover non-finite REAL in CSV (`Inf`/`-Inf`/`NaN` textual form) and JSON (quoted `"Inf"`/`"-Inf"`/`"NaN"`). The grid display must be consistent and unambiguous.
