...
- **Automated**: Byte-golden tests assert CRLF, header, minimal quoting, embedded content, lowercase BLOB hex, empty NULL/text fields, exact numeric tokens, row order, and no warning records.

### Acceptance criteria

- [ ] Given captured rows, then CSV has one header, ascending retained-row order, CRLF records, and minimal RFC 4180 quoting.
- [ ] Given NULL/empty text, BLOB, control characters, or REALs, then each follows the documented CSV representation exactly.
